...
//         showCaption:false,//是否显示标题
        ...
        uploadUrl: '/sysFileUploadInfo/upload/'+path
    	  ...
    	  ajaxPost(url, param, successSaverenyuan, 1000, findError);
      });
	
	$('#kejiju_renyuan_deleteoldFile').click(function() {
		var data = $('#kejiju_renyuan_file_upload_datagrid').bootstrapTable('getAllSelections');
		if(data.length == 0){
			alert("先选中数据");
			...
			callback : function(result) {
					if (result) {
						...
						var param={list:data};
						console.log(param);
						ajaxPost(url, param, deleteSuccessrenyuan, 5000, findError);
					}
				}
			});
		}
...
function reset(){
	// bootstrap table
	$('#kejiju_renyuan_file_upload_datagrid').bootstrapTable({
		url : "/sysFileUploadInfo/findAllBySearchText",
		dataType : "json",
		toolbar : '#kejiju_renyuan_file_upload_toolbar', // 工具按钮用哪个容器
...
$(window).resize(function() {
	$('#kejiju_renyuan_file_upload_datagrid').bootstrapTable('resetView');
});
function deleteSuccessrenyuan(result){
	$('#kejiju_renyuan_file_upload_datagrid').bootstrapTable('refresh');
}

function successSaverenyuan(result){
	$('#kejiju_renyuan_file_upload_datagrid').bootstrapTable('refresh');
}